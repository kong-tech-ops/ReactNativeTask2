...
export const addItemToDatabase = (id, item, isArchived) => {
    db.executeSql(`INSERT INTO ${tableName} (ID, ITEM, ARCHIVED) VALUES(?,?,?)`, [id, item, isArchived]);
    console.log(`${item} with ID ${id} has been added to the database. isArchived: ${isArchived}`);
}

export const archiveItemToDatabase = (id, item, isArchived) => {
    db.executeSql(`UPDATE ${tableName} SET ARCHIVED=? WHERE ID=?;`, [id, isArchived]);
    console.log(`Item name: ${item} with the ID: ${id} was archived in the database. isArchived: ${isArchived}`);
}

export const deleteItemFromDatabase = (id, item) => {
    db.executeSql(`DELETE FROM ${tableName} WHERE ITEM=? AND ID=? `, [item, id]);
    console.log(`Item name: ${item} with the ID: ${id} was deleted from the database.`);
}
